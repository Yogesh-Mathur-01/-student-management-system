...
function getBranch(select) {
    var selectedDepartment = select.value;
    var branchSelect = document.getElementById('branch');

    // Clear previous options for branch
    clearOptions(branchSelect);

    // Add a "Select Branch" option
    var option = document.createElement('option');
    option.text = "Select Branch";
    option.value = "";
    branchSelect.appendChild(option);

    // Make an AJAX request to fetch branches for the selected department
    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'get_branch.php?department=' + selectedDepartment, true);
    xhr.onload = function() {
        if (xhr.status === 200) {
            // Parse the JSON response
            var uniqueBranches = JSON.parse(xhr.responseText);

            // Add unique branches as options
            ...
            // Update the $branch variable with the selected branch
            var selectedBranch = '';
            for (var i = 0; i < branchSelect.options.length; i++) {
                if (branchSelect.options[i].value === '<?php echo $branch; ?>') {
                    selectedBranch = branchSelect.options[i].value;
                    break;
                }
            }
            branchSelect.value = selectedBranch;

            // Call getCourse function if a branch is selected
            if (selectedBranch !== '') {
                getCourse(branchSelect);
            }
        } else {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                ...
                console.error('Error fetching courses:', xhr.status);
            }
        }
    };

    xhr.open('GET', 'get_course.php?department=' + encodeURIComponent(selectedDepartment) + '&branch=' + encodeURIComponent(selectedBranch), true);
    xhr.send();
}
...
function validatesemester(semesterInput) {
    ...
    if (semesterValue == "") {
      ...
      return false;}
    else if (!/^\d{1,8}$/.test(semesterValue)) {
      setError(semesterInput, "semester must contain only digits and should not be greater than 8");
      return false;}
