...
// Check if delete request is made and enrollment number is provided
if (isset($_GET['delete']) && $_GET['delete'] === 'true' && isset($_GET['enroll'])) {
    // Retrieve the enrollment number from the URL
    $enroll_to_delete = $_GET['enroll'];
    
    // Prepare SQL query to delete the record with the provided enrollment number
    $sql_delete = "DELETE FROM student WHERE enroll=?";
    
    // Prepare and execute the delete statement
    $stmt = $conn->prepare($sql_delete);
    $stmt->bind_param("s", $enroll_to_delete);
    $stmt->execute();
    
    // Check if deletion was successful
    if ($stmt->affected_rows > 0) {
        // Deletion successful
        echo '<div class="alert alert-success mt-3">Student record deleted successfully!</div>';
    } else {
        // No records deleted (record not found)
        echo '<div class="alert alert-warning mt-3">No record found to delete.</div>';
    }
}
        ...
        <tbody>
            ...
            ?>
                    <tr>
                        ...
                        <td>
                        <a href="view.php?enroll=<?php echo $row["enroll"]; ?>" class="view-btn">View</a>
                        <a href="update.php?enroll=<?php echo $row["enroll"]; ?>" class="update-btn">Update</a>
                        <a href="?delete=true&enroll=<?php echo $row["enroll"]; ?>" class="delete-btn">Delete</a>
                    </td>
